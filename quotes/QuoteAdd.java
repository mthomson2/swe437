...
public class QuoteAdd {
	...
	/**
	 * Created by Molly Modified by Eric
	 */
	...
	private void initialize() {
		...
		/*
		 * btnAddNew Button is working correctly.
		 */
	...
	private void btnCleanActionPerformed(java.awt.event.ActionEvent evt) {
		// When clean button is click, then clear textArea and unselected all radio
		// button.
	...
	private void btnAddActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
		...
		if (author.length() == 0 || quotes.length() == 0) {
			...
			// case 2: if inputs are not empty, then determine that author and quotes are
			// valied or not.
			...
			if (checkQuote && checkAuthor) {
				...
				// then call add method in ProcessXML class to add author and quotes into XML
				// file.
				add = new processXML();// initialize add value
				add.process(author, quotes);// passing author and quotes into processXML.
				quoteText.setText("Your quote has been saved!!");

			} else {
				// case 4: if author string does not satisfy, then print out error message in
				// quoteText area.
				if (!checkAuthor) {
					quoteText.setText("-------Error------" + '\n'
					...
					// case 5: if quotes string does not satisfy, then print out error message in
					// quoteText area.
	...
	private void btnExitActionPerformed(java.awt.event.ActionEvent evt) {
		// When exit button is clicked, exit the Add Quote frame
		frame.dispose();
	}// end of exit button
	...
	public boolean checkGrammer(String str, int index) {
		...
		if (t1 == str.charAt(index) || t2 == str.charAt(index) || t3 == str.charAt(index) || t4 == str.charAt(index)
				|| t5 == str.charAt(index) || t6 == str.charAt(index)) {
			check = true;
		} else {
			check = false;
		}
	...
	public boolean checkLastName(String str, int index) {
		...
		if (Character.isSpaceChar(str.charAt(index - 1)) && !Character.isUpperCase(str.charAt(index))) {
	...
	public boolean checkAuthor(String str) {
		// checking author name is valid or not.
		int i = str.length();
		char period = '.';
		boolean checkAuthor = false;
	...
	public boolean checkQuote(String str) {
		// checking quote are valid or not.
		int i = str.length();// get string length.
		boolean checkQuote = false;

		for (int index = 0; index < i; index++) {
			// using the loop to access each char in string, then
			// use isLetter() method to check char is alphabet or not.
			if (!Character.isLetter(str.charAt(index))) {

				if (Character.isSpaceChar(str.charAt(index)) || checkGrammer(str, index)) {
					// check if there is space or punctuation mark
					// if its either space or punctuation mark, then return true.
